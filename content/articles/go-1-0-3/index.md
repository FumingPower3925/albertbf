...
The decompressor bug crashed the program. This one produced no crash and no error at all, and it took months to pin down.

### A short signature

In July 2012 someone reported that a Go TLS server worked against Go clients and against browsers, and failed against OpenSSL, roughly one handshake in every few hundred, with nothing to distinguish the connections that broke.[^rsaissue]
