...
The first bug you could trigger from outside the program.

### The crash

You have some compressed bytes from a source you do not control, and you inflate them. Here is that, with a stream built to be malformed. On current Go, hit Run.
...
The decompressor bug crashed the program. This one produced no crash and no error, only an intermittent failure against one client, and it was harder to trace.

### A short signature

In July 2012 Evan Shaw reported that his Go HTTPS server worked against browsers and Go clients but failed against a Ruby client built on OpenSSL, which rejected about one connection in a few hundred with a `bad signature` error and nothing in the failed handshakes to set them apart from the rest.[^rsaissue]
...
The decompressor and the signature were both in the standard library. This bug was in the compiler, and it made a basic operation give the wrong answer.
...
[^rsaissue]: [golang/go issue #3796](https://github.com/golang/go/issues/3796), "crypto/tls: Intermittent errors with OpenSSL client," reported by Evan Shaw (edsrzf) on 3 July 2012. His Go HTTPS server produced occasional RSA signatures that an OpenSSL-based Ruby client rejected with a `bad signature` error; a run of about a thousand requests reliably triggered it.
