...
One thing here I cannot hand you a run of, and it is the headline: the speed. A benchmark is only worth showing on real hardware, and mine is an emulated container and a playground with a faked clock, so any number I produced would be a lie dressed as a measurement. The speedup also lives in the compiler and the runtime, not in your source, which is why it needed no code change and why there is nothing to paste into a playground. So I will cite the figure the Go team reported and spend the runnable part of this piece on what is actually visible: a new tool, and a few sharper edges on the language.
...
One thing this did not change is the default. `GOMAXPROCS` still defaulted to 1 in Go 1.1, the same as in 1.0, so out of the box a program still ran its goroutines through a single P. The rewrite made the runtime able to scale across cores. It did not make it do so on its own; you still raised `GOMAXPROCS` by hand to use more than one.
...
Two writes to `count`, one from the goroutine at line 9 and one from `main` at line 12, with no channel or lock ordering them, and the detector names both. This is a correctness tool, not a speed one. The bug it catches is the kind that survives every reading of the code, because whether it corrupts anything depends on timing that changes from run to run, and a tool that watches every access does not depend on catching the bad interleaving live. It reasons about which accesses could overlap at all. Shipping that in 1.1 put a concurrency-correctness tool in the standard toolchain, next to the concurrency the language had made easy.

The one thing I cannot do is run this on the blog. The playground has no `-race`, so the transcripts above are recorded from a Go 1.0 build and a Go 1.1 build in a period container, with the provenance in the footnote.[^repro]
...
On a current toolchain `g.hello` is a `func() string` closed over `g`, and you can store it, pass it, and call it later. On Go 1.0 the same line does not compile:
...
A current 64-bit toolchain prints 64. Go 1.0 printed 32, and a constant too large for a 32-bit `int` was an error there that is ordinary now:
...
Go 1.1 is the first release where the compatibility promise paid out. A year earlier the team had frozen the language and accepted the cost that went with it, that no mistake in the Go 1 API could ever be removed. What the freeze bought was the freedom to change everything below the language: to replace the scheduler, make the collector precise, and rewrite the compiler's output, then hand all of it back as a recompile that ran a third faster and could now find a program's races. That was the dividend on a year of holding the language still.[^points]
...
[^points]: Two point releases followed, go1.1.1 (13 June 2013) and go1.1.2 (13 August 2013), each a bag of compiler and runtime fixes. Each gets its own entry in this series.
