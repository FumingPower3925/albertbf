...
> [!NOTE]
> You will often read that Go was born while its authors waited 45 minutes for a C++ binary to build. That's close to the truth but not quite it — and Pike, who tells the story, is careful about the distinction. He calls it *"the origin myth for Go,"* and the build in question was *"a major Google binary"* that *"took 45 minutes using a precursor distributed build system."*[^splash] The C++ part is the reader's inference, filled in from the surrounding complaint about header bloat. The frustration was real; the tidy anecdote is, in Pike's own word, a myth.

## The people who showed up

It's worth pausing on who was at that whiteboard, because it explains a great deal about the language that came off it.
...
Go was, in Pike's framing, designed *"in the service of software engineering"* — not as an exercise in language theory but as an answer to specific pain at Google's scale: codebases of tens of millions of lines, worked on by thousands of engineers, with build times that had *"stretched to many minutes, even hours."*[^splash]
...
- **The language should be small.** No header files, no forward declarations — *"everything is declared exactly once."* No type hierarchy: *"types just are."*[^faq]
