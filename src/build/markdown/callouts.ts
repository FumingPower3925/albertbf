...
/**
 * Rewrites GitHub-style callout blockquotes in the token stream:
 *   > [!NOTE]
 *   > body...
 * The blockquote token is tagged and the marker stripped; the renderer
 * override checks the tag.
 */
export function transformCallouts(tokens: Token[]): void {
  for (const token of tokens) {
    if (token.type !== "blockquote") continue;
    const bq = token as Tokens.Blockquote & { calloutKind?: CalloutKind };
    const first = bq.tokens?.[0];
    if (!first || first.type !== "paragraph") continue;
    const para = first as Tokens.Paragraph;
    const match = para.text.match(MARKER_RE);
    if (!match) continue;

    bq.calloutKind = match[1].toLowerCase() as CalloutKind;
    // Strip the marker from the paragraph's raw text and inline tokens.
    para.text = para.text.replace(MARKER_RE, "");
    para.raw = para.raw.replace(MARKER_RE, "");
    if (para.tokens?.[0]?.type === "text") {
      const t = para.tokens[0] as Tokens.Text;
      t.text = t.text.replace(MARKER_RE, "");
      t.raw = t.raw.replace(MARKER_RE, "");
      if (!t.text.trim()) para.tokens.shift();
    }
    if (!para.tokens?.length) bq.tokens.shift();
  }
}
